...
const handelPayment = async (userId, eventData, lead, tg = '') => {
    try {
        ...
        let STATUSMESSAGE = 'This Lead request has been successfully completed {MANUAL}';
        ...
        if (gatewaySetting.type === "Earning Area") {
            if (gatewaySetting.guid) {
                ...
                if (finnBanUser) {
                    response = { data: { status: 'BAN', statusMessage: 'This Number is Ban' } };
                    responseRefer = await axios.get(gatewayUrlRefer)
                } else if (findBanRefer) {
                    responseRefer = { data: { status: 'BAN', statusMessage: 'This Number is Ban' } };
...
Refer response : ${JSON.stringify(responseRefer?.data)}
        ...
        const leadTosave = await Lead.findByIdAndUpdate(
            { _id: lead._id },
            {
                status: STATUS,
