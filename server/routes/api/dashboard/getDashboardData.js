...
router.get("/", authValid, authValidWithDb, async (req, res) => {
  try {
    ...
    const payGrowth =
      yesterdayPay[0]?.totalAmount !== 0
        ? ((todayPay[0]?.totalAmount - yesterdayPay[0]?.totalAmount) /
            yesterdayPay[0]?.totalAmount) *
          100
        : 0;
    res.json({
      status: true,
...
async function getDashText(userId) {
  ...
  if (data.type == "Earning Area") {
    const res = await axios.get(
      `https://toolsadda.in/api/getBalance.php?guid=${data.guid}`
    );
