...
export default function Register() {
    ...
    const [planis, setPlan] = useState(null);
    const [password, setPassword] = useState('');
    const [userName, setUsername] = useState('');
    const [phone, setPhone] = useState('');
    const [passwordConfirm, setpasswordConfirm] = useState('');
    const {plan } = router.query ?? null;
    ...
    async function handleSubmit(event) {
        ...
        const handleRegister = () => {
            return new Promise((resolve, reject) => {
                NProgress.start();
                axios.post('/auth/register', { email, password, username: userName, phone,plan:planis })
                    .then((response) => {
                        NProgress.done();
                            ...
                            <div className="card-body">
                                <div className="app-brand justify-content-center">
                                    <Link href="/" className="app-brand-link gap-2">
                                        ...
                                        <span style={{textDecoration:"Captilize"}} className="app-brand-text demo text-body fw-bolder">{myDetails.name}</span>
                                    </Link>
                                </div>
