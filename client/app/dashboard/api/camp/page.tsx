...
export default function CampApiPage() {
  ...
  const endpointUrl = `${baseUrl}/api/v1/external/campaign/${apiKey}`;
  ...
  const handleTest = async () => {
    if (!campaignId) {
      toast.error("Please enter Campaign ID");
      return;
    }
    setIsLoading(true);
    try {
      const res = await authFetch(`${apiConfig.baseUrl}/api/v1/external/campaign/${apiKey}?camp=${campaignId}`);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Dashboard / <span className="text-foreground">Api</span> / Camp
              Details
            </p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label className="text-sm font-medium">
                campid <span className="text-destructive">*</span>
              </Label>
              <div className="bg-muted/50 p-3 rounded-lg">
                <code className="text-sm">string</code>
                <p className="text-xs text-muted-foreground mt-1">
                  Campaign ID to get details
                </p>
              </div>
            </div>
          </div>
                      ...
                      <div className="grid gap-4 md:grid-cols-2 border rounded-lg p-4">
                        <div className="space-y-1">
                          ...
                          <p className="font-medium font-mono">
                            {result.data.campId || result.data._id || "-"}
                          </p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm text-muted-foreground">
                        ...
                        <div className="space-y-1">
                          ...
                          <Badge
                            variant={
                              result.data.campStatus ? "default" : "secondary"
                            }
                          >
                            {result.data.campStatus ? "Active" : "Inactive"}
                          </Badge>
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm text-muted-foreground">
