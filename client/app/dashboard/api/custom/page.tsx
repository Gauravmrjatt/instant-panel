...
export default function CustomApiPage() {
  ...
  const handleTest = async () => {
    if (!formData.campaign || !formData.event || !formData.number) {
      toast.error(
        "Please fill in required fields: Campaign ID, Event, and Number",
      );
      return;
    }
    setIsLoading(true);
    try {
      ...
      setResult(data);
      if (data.status) {
        toast.success("Custom amount created successfully!");
      } else {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Dashboard / <span className="text-foreground">Api</span> / Custom
              Amount
            </p>
          </div>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label className="text-sm font-medium">userAmount</Label>
              <div className="bg-muted/50 p-3 rounded-lg">
                <code className="text-sm">number</code>
                <p className="text-xs text-muted-foreground mt-1">
                  Custom user amount (optional)
                </p>
              </div>
            </div>
            <div className="space-y-2">
              <Label className="text-sm font-medium">referAmount</Label>
              <div className="bg-muted/50 p-3 rounded-lg">
                <code className="text-sm">number</code>
                <p className="text-xs text-muted-foreground mt-1">
                  Custom refer amount (optional)
                </p>
              </div>
            </div>
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                type="number"
                placeholder="Custom user amount"
                value={formData.userAmount}
                onChange={(e) =>
                  setFormData((p) => ({ ...p, userAmount: e.target.value }))
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                type="number"
                placeholder="Custom refer amount"
                value={formData.referAmount}
                onChange={(e) =>
                  setFormData((p) => ({ ...p, referAmount: e.target.value }))
                }
              />
            </div>
          </div>
