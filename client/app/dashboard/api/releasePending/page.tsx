...
export default function ReleasePendingPage() {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      setResult(data);
      if (data.status) {
        toast.success(data.msg || "Released successfully!");
      } else {
        toast.error(data.msg || "Failed to release");
      }
    } catch (error) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Dashboard / <span className="text-foreground">Api</span> / Release
              Pending
            </p>
          </div>
        </div>
