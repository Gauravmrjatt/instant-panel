...
async function processBatch(batch, userData, event, updateStatus) {
  const promises = batch.map((id) =>
    axios
      .get(
        `/api/v1/postback/${userData.key}/${event}?click=${id.value}&type=manual`
      )
      .then(() => updateStatus(id.value))
      .catch((error) => {
        console.log(error);
        updateStatus(id.value); // Update even if the request fails
      })
  );
...
export default function ClickIdPayList({ list, paying, event, setDisable }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (paying === true && !updatedRef.current) {
        ...
        // Process items in batches of 5 or 6
        const batchSize = 5; // Change this to 6 if needed
        for (let i = 0; i < items.length; i += batchSize) {
          const batch = items.slice(i, i + batchSize);
          await processBatch(batch, userData, event, updateStatus);
        }
  ...
  return (
    <>
      <div
        style={{ maxHeight: "400px", overflow: "scroll", overflowX: "hidden" }}
      >
        <LinearProgress
          sx={{ borderRadius: "20px" }}
          variant="buffer"
          value={percentage}
          valueBuffer={percentage2}
        />
        <List sx={{ width: "calc(100% - 10px)", bgcolor: "background.paper" }}>
          {items.map((item) => (
            <ListItem
            ...
            >
              <ListItemText
                sx={{ marginBlock: "15px", fontSize: "2.125rem" }}
                primary={item.value}
              />
              {item.status ? <Loading type="spinner" /> : null}
            </ListItem>
      ...
      >
        <small>
          <b>{doneCount.current.count}</b> of {items.length}
        </small>
        <small>
          Total ClickId: <b>{items.length}</b>
        </small>
      </div>
    </>
  );
